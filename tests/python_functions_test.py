...
class TestPythonFunctionPractice(unittest.TestCase):
  ...
  def test_volume_of_cube(self):
    vol_return = vol_of_cube(2)
    self.assertEqual(8, vol_return)
  ...
  def test_reverse_string(self):
    #add test code here
    reversed_string = reverse_string("codeclan")
    self.assertEqual("nalcedoc", reversed_string)

  #Given a value in farenheit, convert this into celsius.
  
  def test_fahrenheit_to_celsius(self):
    #add test code here
    celsius = fah_to_cel(41)
    self.assertEqual(5, celsius)
